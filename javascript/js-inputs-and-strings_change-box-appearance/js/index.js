...
colorInput.addEventListener("input", () => {
  ...
  box.style.backgroundColor = `hsl(${inputValue},70%,60%)`;
});

borderRadiusInput.addEventListener("input", () => {
  const inputValue = borderRadiusInput.value;
  box.style.borderRadius = `${inputValue}px`;
});

rotationInput.addEventListener("input", () => {
  const inputValue = rotationInput.value;
  box.style.transform = `rotate(${inputValue}deg)`;
});
